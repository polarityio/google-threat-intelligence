  ...
  requestWithDefaults(requestOptions, function (err, response, body) {
    _handleRequestError(err, response, body, options, function (err, responseBody) {
      ...
      const vulnerabilitiesLookupResult = {
        entity,
        data: {
          summary: [`Vulns: ${responseBody.data.length}`],
          details: {
...
const formatVulnerabilities = (vulnerabilitiesResponseBody) => flow(
  get('data'),
  map((vulnerability) => ({
    ...
    cvssScore: flow((attrs) => {
      const cvss3Score =
        get('cvss.cvssv3_1.base_score', attrs) || get('cvss.cvssv3_x.base_score', attrs);
    ...
    timeToMitigation: flow((attrs) => {
      ...
      const fixDate = flow(get('vendor_fix_references'), (fixes) =>
        fixes && fixes.length
          ? Math.min(
              ...fixes
                .map((f) => (get('published_date', f) ? f.published_date * 1000 : null))
                .filter(Boolean)
            )
          : null
      )(attrs);
    ...
    _handleRequestError(err, response, body, options, function (err, result) {
      ...
      const threatActorsLookupResult = {
        ...
        data: {
          ...
          details: {
            threatActors: formatThreatActors(result)
          }
        }
      };
